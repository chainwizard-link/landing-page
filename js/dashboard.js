...
async function createTask( _modelHash = ""){
    ...
    let resp = await chainwizardContract.createTask(_modelHash, rndCount);
    console.log(resp);
    return resp;
    ...
    //             text: 'Your can try chainwizard out by using the provided sample model file.'
    //         })
    //         rej("Invalid File");
    //     }
    //     else{
...
async function getTasks(_userAddress) {
    ...
    data.forEach(async function(log){
        ...
        let logData = chainwizardContract.interface.decodeEventLog(frag, log.data, log.topics);
