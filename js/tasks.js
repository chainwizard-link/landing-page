...
async function copyLatest(){
    ...
    let filter2 = chainwizardContract.filters.modelUpdated(parseInt(_taskId));
    let frag2 = chainwizardContract.interface.getEvent('modelUpdated');
    let logs = await chainwizardContract.queryFilter(filter2, 0x0);


    if (logs.length > 0){
        let log = chainwizardContract.interface.decodeEventLog(frag2, logs[logs.length-1].data, logs[logs.length-1].topics);
        console.log(log);
        Swal.fire({
            icon: 'success',
        ...
        let filter = chainwizardContract.filters.newTaskCreated(null, accounts[0]);
        let frag = chainwizardContract.interface.getEvent('newTaskCreated')
        let data = await chainwizardContract.queryFilter(filter, 0x0);

        logs = chainwizardContract.interface.decodeEventLog(frag, data[0].data, data[0].topics);
...
async function viewModelLinks(){
    ...
    let result = await chainwizardContract.chainwizardTasks(_taskId);
    let resultDict = {
...
async function getTaskHashes(_taskId = 1, _userAddress = accounts[0]) {
    ...
    let filter = chainwizardContract.filters.newTaskCreated(null, _userAddress);
    let frag = chainwizardContract.interface.getEvent('newTaskCreated')
    let data = await chainwizardContract.queryFilter(filter, 0x0);

    let logData = chainwizardContract.interface.decodeEventLog(frag, data[0].data, data[0].topics);
    ...
    let filter2 = chainwizardContract.filters.modelUpdated(_taskId);
    let frag2 = chainwizardContract.interface.getEvent('modelUpdated');
    let data2 = await chainwizardContract.queryFilter(filter2, 0x0);
    let i = 2;
    data2.forEach((log)=>{
        logf = chainwizardContract.interface.decodeEventLog(frag2, log.data, log.topics);
